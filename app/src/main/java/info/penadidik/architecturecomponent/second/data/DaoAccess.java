...
public interface DaoAccess {
    ...
    @Query("SELECT * FROM " + MyRoomDb.TABLE_NAME_TODO)
    List<Todo> fetchAllTodos();

    @Query("SELECT * FROM " + MyRoomDb.TABLE_NAME_TODO + " WHERE category = :category")
    List<Todo> fetchTodoListByCategory(String category);

    @Query("SELECT * FROM " + MyRoomDb.TABLE_NAME_TODO + " WHERE todo_id = :todoId")
